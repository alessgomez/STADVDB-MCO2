...
app.get('/', async (req, res) => {
   ...
   } catch (error) { //Node 0 cannot begin transac, load from Node 1 and Node 2
      console.log(error)
      try {
         ...
         .then (async data1 => {
            ...
            } catch (error) { //Node 2 cannot begin transac, cannot load half of the data 
               console.log(error)
            }
         })
      } catch(error) { //Node 1 cannot begin transac, load half from node 2
         console.log(error)
         try {
            await db[2].beginTransaction();
            const query = "SELECT * FROM movies";
...
app.get('/search', async(req, res) => {
   ...
   } catch (error) { //node 0 cannot begin transac, search in node 1
      try {
         await db[1].beginTransaction();
         const query = `SELECT * FROM movies WHERE ${req.query.attribute} = "${req.query.value}"`
         db[1].query(query)
         ...
         } catch (error) { //node 2 cannot begin transac, cannot search
            console.log(error)
         }
      }
   }
